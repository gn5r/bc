...
export class UserIconCommand extends AbstractSubCommand<
  {},
  UserIdOption & OutputOption
> {
  command = "userIcon";
  describe =
    "ユーザーのアイコン画像を取得します。\nhttps://developer.nulab.com/ja/docs/backlog/api/2/get-user-icon/";
  protected build = (argv: Argv<{}>) =>
    withUserIdOption(withOutputOption(argv));
  protected async execute(
    args: ArgumentsCamelCase<UserIdOption & DebugOption & OutputOption>
  ): Promise<void> {
    const userIcon: Entity.File.FileData = await this.client.getUserIcon(
      args.userId!
    );
    ...
    const outputPath = args.output
      ...
      : path.resolve(
          process.cwd(),
          (userIcon as Entity.File.NodeFileData).filename
        );
